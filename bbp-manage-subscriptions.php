...
class BBPMS_List_Table extends WP_List_Table
{
	...
	$forums_all_data = get_forum_data();
        ...
        $columns = $this->get_columns($roles_to_show,  $forums_all_data['all_ids_array']);
        ...
        $data = $this->table_data($roles_to_show, $forums_all_data['all_ids_array'], $userid);
     ...
     * @return Array
     */
    function get_columns($roles_to_show, $all_ids_array)
    {
        $columns = array(
            'display_name'	=> 'Name'
        );
	...
	foreach ($all_ids_array as $forum_id) {
		...
		if ($forum_parent_id) {
			$forum_title = $forum_title . '<br>(' . bbp_get_forum_title($forum_parent_id) . ')';
		}
     ...
     * @return Array
     */
    function table_data($roles_to_show, $forums_to_show, $userid)
    {
	global $wpdb;
